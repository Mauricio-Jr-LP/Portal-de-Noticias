...
function HomeScreen({navigation}) 
{
  ...
  return (
    <View style={{flex:1}}>
      <View style={{ flex:0.3 }}>
        <ScrollView horizontal contentContainerStyle={{width:'200%',height:'100%',backgroundColor:'white',}} style={{flex:1}}>
          <StatusBar hidden/>
          {
      ...
      <View style={{flex:0.7,padding:20}}>
        <View style={{width:50,height:2,backgroundColor:'#2D2525',position:'absolute', left:40,top:40}}></View>
        <Text style = {{color: 'black'}}>Mais Notícias</Text>
              ...
              if(index >= 2)
              {
                return (
                  <View style={{flexDirection:'row',marginBottom:10}}>
                    <TouchableOpacity style={{flexDirection:'row', backgroundColor: 'rgba(38, 38, 38, 0.5)'}} onPress={()=>navigation.navigate('Noticia',
                      {
                        titulo: val.info.titulo,
                        conteudo: val.info.conteudo,
                        imagem: val.info.imagem
                      ...
                      <Text style={{padding:10, color: 'white'}}>{val.info.titulo}</Text>
                    </TouchableOpacity>
                </View>
              )}
            })
...
function App() 
{
  return (
    <NavigationContainer theme={MyTheme} >
      <Stack.Navigator >
        <Stack.Screen name="Castelo Noticias" component={HomeScreen}/>
